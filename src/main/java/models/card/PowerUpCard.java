...
public class PowerUpCard extends EffectCard {
    ...
    public void payPrice(Ammo yourAmmo, Ammo whichAmmo, PowerUpCard powerUpCard) {
        ...
        if (whichAmmo != null && (whichAmmo.red + whichAmmo.yellow + whichAmmo.blue) != 1)
            throw new WeaponCardException("You have to pay exactly one ammo of any color");
    ...
    public void activate() {
        ...
        if (hasPrice && !pricePaid)
            throw new WeaponCardException("You need to pay this powerup price to active it. card: " + this.name);
        this.activated = true;
