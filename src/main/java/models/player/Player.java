...
public class Player implements Serializable, Taggable, Observable {
    ...
    public void drawWeaponCard(WeaponCard drawn, PowerUpCard powerUpToPay, WeaponCard toRelease) {
        if (powerUpToPay != null && !powerUpList.contains(powerUpToPay))
            throw new WeaponCardException("You do not have this powerup: " + powerUpToPay.name);
        ...
        if (weaponList.size() == 3) {
            ...
            if (!weaponList.contains(toRelease))
                throw new CheatException("You do not have the card you are trying to release");
    ...
    public int getMarksFromPlayer(Player fromWho) {
        return this.marks.getMarksFromPlayer(fromWho);
    }

    void removeAllMarkFromPlayer(Player fromWho) {
        this.marks.removeAllMarkFromPlayer(fromWho);
    }
    ...
    public void playPowerUp(PowerUpCard powerUpCard, Ammo ammo, PowerUpCard paymentPowerUpCard) {
        ...
        if (paymentPowerUpCard != null) {
            ...
            if (paymentPowerUpCard.equals(powerUpCard))
                throw new WeaponCardException("You cannot use the power up you are playing to activate it");
        }
        ...
        switch (powerUpCard.when) {
            case "on_damage_dealt":
                if (playersDamagedByMeThisTurn.size() == 0)
                    throw new WeaponCardException("You can only play this card if you damaged someone in this turn");
                break;
            case "my_turn":
                if (!(match.getCurrentPlayer().equals(this) && !match.isDoingAction()))
                    throw new WeaponCardException("You can only play this powerup in your turn");
                break;
            case "on_damage_received":
                Player lastDamager = getLastDamager();
                if (lastDamager == null)
                    throw new WeaponCardException("You did not receive any damages so you cannot activate this powerup");
                if (!lastDamager.equals(match.getCurrentPlayer()))
                    throw new WeaponCardException("You did non receive any damages in this turn so you cannot activate this powerup");
                break;
            default:
    ...
    private void checkHasWeapon(WeaponCard weaponCard) {
        if (!weaponList.contains(weaponCard))
            throw new WeaponCardException("This user does not have this weapon:" + weaponCard.name);
    }

    private void checkHasPowerUp(PowerUpCard powerUpCard) {
        if (!powerUpList.contains(powerUpCard))
            throw new WeaponCardException("This user does not have this powerup:" + powerUpCard.name);
    }
